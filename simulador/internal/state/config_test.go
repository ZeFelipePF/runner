...
func TestConfig_DefaultValues(t *testing.T) {
	...
	assert.Equal(t, PortaPadraoSimulador, cfg.Simulador.PortaPadrao)
...
func TestConfig_LoadPartial_FillsDefaults(t *testing.T) {
	...
	partial := map[string]any{
		"simulador": map[string]any{"portaPadrao": 8888},
	}
	...
	cfg, err := LoadConfig()
	require.NoError(t, err)
	assert.Equal(t, 8888, cfg.Simulador.PortaPadrao)
	assert.Equal(t, 8088, cfg.Assinador.PortaPadrao)
	assert.Equal(t, 21, cfg.JDK.VersaoMinima)
}
...
func TestConfig_SaveAndLoad(t *testing.T) {
	...
	cfg := DefaultConfig()
	cfg.Simulador.PortaPadrao = 7777
	require.NoError(t, cfg.SaveConfig())

	loaded, err := LoadConfig()
	require.NoError(t, err)
	assert.Equal(t, 7777, loaded.Simulador.PortaPadrao)
}
